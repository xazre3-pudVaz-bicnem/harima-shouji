    ...
    <div style={{ background: '#F6F4EF' }}>
      ...
      <section style={{ background: '#FFFFFF', paddingTop: '7rem', paddingBottom: '7rem' }}>
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '0 2rem' }}>
          <div style={{ fontFamily: 'var(--font-mono)', fontSize: '0.625rem', fontWeight: 500, letterSpacing: '0.24em', color: '#8F8B82', textTransform: 'uppercase', marginBottom: '1rem' }}>ALL CASES</div>
          ...
          <div style={{ display: 'grid', gap: '2px' }} className="grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {cases.map((c) => (
              <Link key={c.slug} href={`/cases/${c.slug}`} style={{ display: 'block', textDecoration: 'none', background: '#EDEAE2' }}>
                ...
                <div style={{ padding: '1.75rem 1.5rem' }}>
                  <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '0.75rem' }}>
                    <span style={{ fontSize: '0.6875rem', fontWeight: 600, color: '#57544D', background: '#FFFFFF', padding: '0.2rem 0.6rem' }}>{c.serviceType}</span>
                    <span style={{ fontSize: '0.6875rem', fontWeight: 600, color: '#8F8B82', background: '#FFFFFF', padding: '0.2rem 0.6rem' }}>{c.industry}</span>
                  </div>
                  <h3 style={{ fontSize: '0.9375rem', fontWeight: 700, color: '#0A0A0A', lineHeight: 1.5, marginBottom: '0.5rem' }}>{c.title}</h3>
                  <div style={{ fontSize: '0.75rem', color: '#8F8B82' }}>{c.area} / 工期: {c.duration}</div>
                </div>
              </Link>
            ))}
          </div>
